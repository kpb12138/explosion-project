...
def run_optimization_algorithms(objective_func, bounds, initial_guess, param_names, step_name, config, results_dir, residuals_func=None):
    ...
    if use_gp_minimize and HAS_GP:
        ...
        step_results = gp_minimize(
            objective_func, tuple_bounds, x0=initial_guess, n_calls=n_calls,
    ...
    if use_least_squares:
        print(f"Running least_squares for {step_name} optimization with initial guess from {'genetic algorithm' if use_genetic else 'config'}")
        ...
        ls_func = residuals_func if residuals_func is not None else lambda p: np.array([objective_func(p)])
        # Use callback only if scipy version supports it
        try:
            step_results = least_squares(
                ls_func, current_initial_guess,
                bounds=(ls_lower, ls_upper), verbose=2, ftol=1e-6, callback=ls_callback, max_nfev=50
            )
        except TypeError:
            step_results = least_squares(
                ls_func, current_initial_guess,
                bounds=(ls_lower, ls_upper), verbose=2, ftol=1e-6, max_nfev=50
            )
