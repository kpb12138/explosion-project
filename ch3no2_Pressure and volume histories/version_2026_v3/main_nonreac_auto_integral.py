...
from residuals_nonreac_integral import objective_1st, objective_hi, objective_total, residuals_vector_total, DeltaP_residuals

def main():
    # create results directory
    results_dir = 'phi_0.5 2 15bar_result'
    ...
    config = load_config('phi_0.5 2 15bar.json')
    ...
    if not idts_1st:
        ...
        A1_range = (optimized_A1 * (1 - A1_factor), optimized_A1 * (1 + A1_factor))
        n1_range = (optimized_n1 * (1 + n1_factor), optimized_n1 * (1 - n1_factor))
        E1_range = (optimized_E1 * (1 - E1_factor), optimized_E1 * (1 + E1_factor))
    ...
    if not idts_one:
        ...
        Ah_range = (optimized_Ah * (1 - Ah_factor), optimized_Ah * (1 + Ah_factor))
        nh_range = (optimized_nh * (1 + nh_factor), optimized_nh * (1 - nh_factor))
        Eh_range = (optimized_Eh * (1 - Eh_factor), optimized_Eh * (1 + Eh_factor))

    # Step 3: optimize Teq, k, w using temperature, pressure and correspoding Dp
    dp_data = config['file_specifications']['dp_optimization']
    T_list = np.array(dp_data.get('initial_temperature', []))
    P_list = np.array(dp_data.get('initial_pressure', []))
    dp_actual = np.array(dp_data.get('D_p_actual', []))
    ...
    print("\n所有优化步骤完成！结果保存至:", os.path.abspath(results_dir))
